<script lang="ts">
	...
	// Create the stateful variables
	let legal_name = $state('');
	let country = $state('United States');

	const saveBusiness = async (event: Event) => {
		...
		// Redirect on success
		window.location.href = '/onboarding/bank';
	};
</script>
...
<main class="mx-auto flex max-w-4xl flex-col items-center justify-center">
	<div class="steps mb-8 w-full">
		<ul class="steps text-sm">
			...
			<li class="step step-primary">Business</li>
			<li class="step">Bank</li>
		</ul>
	</div>
	<div class="page-header w-full">
		<h1 class="text-2xl">Create a Business</h1>
		<p class="text-sm">Create a business to get started with your account.</p>
	</div>

	<form class="mt-8 flex w-full flex-col gap-4" method="POST" onsubmit={saveBusiness}>
		<div class="form-control w-full">
			<label class="label" for="business_name">
				<span class="label-text">Business's Legal Name</span>
			</label>
			<input
				type="text"
		...
		<div class="form-control">
			...
			<select
				id="country"
				name="country"
				class="input input-bordered select w-full"
				required
				aria-label="Country"
				aria-required="true"
				bind:value={country}
		...
		<button class="btn btn-primary mt-4" type="submit"> Create Business </button>
	</form>
</main>
