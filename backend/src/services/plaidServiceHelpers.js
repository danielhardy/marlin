...
export async function upsertPlaidItem(itemData) {
  const row = {
    business_id: itemData.business_id, // UUID, FK to businesses.id
    item_id: itemData.id, // TEXT, Plaid's item_id, UNIQUE
    access_token: itemData.access_token, // TEXT
    cursor: itemData.cursor || null, // TEXT
    last_synced_at: itemData.last_synced_at || null, // TIMESTAMPTZ
    active: typeof itemData.active === "boolean" ? itemData.active : true, // BOOLEAN
    institution_id: itemData.institution_id || null, // TEXT
    institution_name: itemData.institution_name || null, // TEXT
    products: itemData.products || [], // TEXT[]
    // 'id' (UUID PK) and 'created_at' are auto-generated by the database.
    // 'last_failed_at' can be updated separately.
  };
  console.log(
    `Upserting Plaid item (Plaid's textual ID: ${row.item_id}) for business ${row.business_id}...`
  );
  try {
    // 'item_id' (Plaid's textual ID) is the conflict target due to its UNIQUE constraint.
    const { data: upsertedData, error } = await supabaseAdmin
      .from("plaid_items")
      .upsert(row, { onConflict: "item_id" }) // onConflict uses Plaid's textual item_id
      .select() // Select the full upserted row
      .single(); // Expect a single row to be returned

    if (error) throw error;
    console.log(
      `Plaid item (Plaid's textual ID: ${row.item_id}, DB UUID PK: ${upsertedData.id}) upserted successfully.`
    );
    return upsertedData; // Return the full row, including the database 'id' (UUID PK)
  } catch (error) {
    console.error(
      `Error upserting Plaid item (Plaid's textual ID: ${row.item_id}):`,
      error
    );
    throw error;
  }
